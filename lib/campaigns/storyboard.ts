...
export const SAFE_ZONE_STRONG_CLAUSE =
  ' STRICT SAFE ZONE - this is a hard requirement, not a preference: even though the frame is a tall 9:16, treat the central 4:5 region (the middle, roughly the central two-thirds vertically) as the only place where important content may go. The ENTIRE product and the main person face and upper body must sit fully inside that central 4:5 region, with clearly empty background filling roughly the top sixth and the bottom sixth of the frame. Keep the product comfortably small: its top edge and especially its BOTTOM edge must stay well inside the central 4:5 with margin to spare - do not let the bottom of the product drop into the lower part of the frame. If in doubt, make the product smaller rather than larger: a product whose top or bottom overflows the 4:5 is a failure, while extra empty margin is perfectly fine. Never crop the product. The shot has to look intentional and complete both as a full 9:16 and when cropped to the central 4:5.';
