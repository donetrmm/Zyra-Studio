...
- **Smoke del usuario (con key real):** (1) `submitStudioTurnAction` con `provider:'nano-banana'` → job encolado → worker genera → generación `done` con output en Storage y créditos confirmados. (2) Lo mismo con `provider:'gpt-image', model:'gpt-image-1'`. (3) **Compuerta de edición gpt-image por gateway:** un turno gpt-image con `referenceIds` (edición) → si el gateway NO rutea la edición, activar el fallback documentado en el spec (`OPENAI_API_KEY` + `@ai-sdk/openai`) en una tarea aparte. (4) Confirmar en el deploy que `maxDuration=300` toma efecto en la org de Vercel.
