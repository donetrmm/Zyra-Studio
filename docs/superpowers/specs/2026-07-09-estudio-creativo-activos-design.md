...
- **Edición GPT Image por el gateway: probable pero SIN confirmar end-to-end.** Docs recientes dicen que sí (hasta 4 refs vía `prompt.images`), pero un hilo de Vercel de mediados de 2026 indicaba que el gateway aún no ruteaba el endpoint de edición de gpt-image. → **Compuerta:** smoke test de edición `openai/gpt-image-2` con `prompt.images` contra el gateway con key real. **Decisión del usuario (firme): ambos proveedores (Nano y gpt-image) CREAN Y EDITAN; el usuario elige cuál usar por turno. TODO por el Vercel AI Gateway, sin `OPENAI_API_KEY` directo.** El smoke confirma la edición gpt-image por gateway (esperada por docs); no se planea degradar gpt-image a generación-only.
...
- **Compuerta de edición gpt-image por gateway:** smoke test con key real. Ambos proveedores editan por diseño (elección del usuario); el smoke confirma la edición gpt-image por gateway. **Sin fallback a OpenAI directo (decisión firme: todo por el gateway).**
...
- **Transversal / compuerta:** smoke test de edición gpt-image por gateway (ambos proveedores editan; elección del usuario). Sin fallback a key directa de OpenAI.
