...
2. **Ambos proveedores desde v1:** Nano Banana (ya integrado) y GPT Image, con selector de modelo.
...
> **Supuesto/dependencia crítica (flag):** un job de imagen se resuelve en **una sola invocación** del worker (llama al proveedor bloqueante, sube, completa), sin re-encolado por polling — el proveedor de imagen no se puede sondear en trozos como el video, la llamada bloquea hasta terminar. Esto **exige el timeout de función de 300s** (adoptado en las decisiones de Next 16 y default actual de Vercel), **no** el límite de 60s de Vercel Hobby que menciona `docs/zyra-studio-spec.md`. Si el despliegue efectivo siguiera en 60s, ninguna llamada de proveedor >60s (todas las de gpt-image y las de Nano lentas) completaría en una invocación → habría que rediseñar (proveedor con submit+poll, o acotar a modelos rápidos). **Verificar el `maxDuration` real del worker en producción antes de implementar.** `gpt-image-2` en "alta" (~280s) queda cerca del techo de 300s → best-effort (ver Errores); default interactivo = media.
...
- **Compuerta de edición gpt-image por gateway:** smoke test con key real antes de habilitar la edición gpt-image; si falla, fallback a `OPENAI_API_KEY` directo solo para ese modo (documentado como desviación consciente).
...
- **Transversal / compuerta:** smoke test de edición gpt-image por gateway (con fallback a key directa).
